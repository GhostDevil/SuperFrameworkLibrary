...
namespace SuperFramework.SuperFTP
{
    ...
    public class FTPClient
    {
        ...
        public string[] GetDirList(string strMask)
        {
            if (!bConnected)
            {
                Connect();
        ...
        public void DownFiles(string strFileNameMask, string strFolder)
        {
            if (!bConnected)
            {
                Connect();
            }
            string[] strFiles = GetDirList(strFileNameMask);
            foreach (string strFile in strFiles)
            {
                if (!strFile.Equals(""))//一般来说strFiles的最后一个元素可能是空字符串
                {
                    DownFile(strFile, strFolder, strFile);
                }
            }
        }
        ...
        public void DownFile(string strRemoteFileName, string strFolder, string strLocalFileName)
        {
            Socket socketData = CreateDataSocket();
            try
            {
        ...
        /// </summary>
        /// <param name="strFileName">本地文件名</param>
        /// <param name="strNewFileName">上传之后新的文件名</param>
        public void PutByGuid(string strFileName, string strNewFileName)
        {
            if (!bConnected)
            {
                Connect();
            ...
            string str = strFileName.Substring(0, strFileName.LastIndexOf("\\"));
            string strTypeName = strFileName.Substring(strFileName.LastIndexOf("."));
            strNewFileName = str + "\\" + strNewFileName;
            System.IO.File.Copy(strFileName, strNewFileName);
            System.IO.File.SetAttributes(strNewFileName, System.IO.FileAttributes.Normal);
            Socket socketData = CreateDataSocket();
            SendCommand("STOR " + Path.GetFileName(strNewFileName));
            if (!(iReplyCode == 125 || iReplyCode == 150))
            {
                throw new IOException(strReply.Substring(4));
            }
