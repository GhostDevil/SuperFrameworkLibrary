...
namespace SuperFramework.SuperEncrypt
{
    ...
    public class SM2Encrypt
    {
        ...
        public SM2Encrypt(string pubkey, string privkey, Mode mode)
        {
            if (pubkey != null) this.pubkey = Decode(pubkey);
            if (privkey != null) this.privkey = Decode(privkey);
            this.mode = mode;
        }
        public SM2Encrypt(byte[] pubkey, byte[] privkey, Mode mode)
        {
            this.pubkey = pubkey;
        ...
        readonly byte[] pubkey;
        readonly byte[] privkey;
        readonly Mode mode;
        ...
        ICipherParameters PrivateKeyParameters
        {
            get
            {
                ...
                if (r == null) 
                    r = _privateKeyParameters = new ECPrivateKeyParameters(new BigInteger(1, privkey), new ECDomainParameters(GMNamedCurves.GetByName("SM2P256V1")));
                return r;
            }
        }
        ...
        /// 签名 (SM3WithSM2签名)
        /// </summary>
        /// <param name="msg">待签名数据</param>
        /// <param name="id">用户身份标识 默认  Encoding.ASCII.GetBytes("1234567812345678")</param>
