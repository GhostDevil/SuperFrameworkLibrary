...
namespace SuperFramework.IDCard
{
    /// <summary>
    /// 日期:2015-06-24
    /// 作者:不良帥
    ...
    public class IDCardHelper
    {
        ...
        public static string Validate(string idcard)
        {
            ...
            else if (idcard.Length == 18)
            {
                ...
                for (int i = 0; i < 17; i++)
                {
                    int tmp = int.Parse(idcard.Substring(i, 1));
                    ll_sum += tmp * li_quan[i];
                }
                ll_sum %= 11;
                if (idcard.Substring(17, 1) != ls_jy[ll_sum])
                {
                    return string.Format("身份证号码最后一位应该是{0}！", ls_jy[ll_sum]);
                }
            }
            ...
            try
            {
                DateTime.Parse(string.Format("{0}-{1}-{2}", idcard.Substring(6, 4), idcard.Substring(10, 2), idcard.Substring(12, 2)));
            }
            catch
            {
                return "非法生日";
        ...
        public static string IdCard15To18(string idcard)
        {
            ...
            {
                int ll_sum = 0;
                string result = string.Format("{0}19{1}", idcard.Substring(0, 6), idcard.Substring(6, 9));
                for (int i = 0; i < 17; i++)
                {
                    int tmp = int.Parse(result.Substring(i, 1));
                    ll_sum += tmp * li_quan[i];
                }
                ll_sum %= 11;
                result += ls_jy[ll_sum];
        ...
        public static DateTime GetBirthday(string id)
        {
            ...
            if (id.Length == 15)
            {
                result = string.Format("19{0}-{1}-{2}", id.Substring(6, 2), id.Substring(8, 2), id.Substring(10, 2));
            }
            else if (id.Length == 18)
            {
                result = string.Format("{0}-{1}-{2}", id.Substring(6, 4), id.Substring(10, 2), id.Substring(12, 2));
            }
            else
            {
                throw new ArgumentException("身份证号码不是15或者18位！");
